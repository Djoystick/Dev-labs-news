...
function InlineError({ actionLabel = 'РџРѕРІС‚РѕСЂРёС‚СЊ', message, onAction }: { actionLabel?: string; message: string; onAction: () => void }) {
  return (
...
export function TopicPreferencesPage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([fetchTopics(controller.signal), fetchMyTopicIds(user.id, controller.signal)])
      ...
      .catch((error) => {
        if (!controller.signal.aborted) {
          setLoadError(error instanceof Error ? error.message : 'РќРµ СѓРґР°Р»РѕСЃСЊ Р·Р°РіСЂСѓР·РёС‚СЊ РЅР°СЃС‚СЂРѕР№РєРё СЂР°Р·РґРµР»РѕРІ.');
        }
      })
      .finally(() => {
        if (!controller.signal.aborted) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveError(error instanceof Error ? error.message : 'РќРµ СѓРґР°Р»РѕСЃСЊ СЃРѕС…СЂР°РЅРёС‚СЊ РЅР°СЃС‚СЂРѕР№РєРё СЂР°Р·РґРµР»РѕРІ.');
    } finally {
      setSaving(false);
    }
  };
    ...
    <FlatPage className="safe-pb safe-pt flex min-h-full flex-col pb-36 pt-6 sm:pb-40 sm:pt-8">
      <FlatSection className="pt-0">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.28em] text-primary">РђРєРєР°СѓРЅС‚</p>
            <h1 className="mt-2 text-4xl font-bold sm:text-5xl">РќР°СЃС‚СЂРѕР№РєРё СЂР°Р·РґРµР»РѕРІ</h1>
          </div>
          <Button type="button" variant="ghost" size="icon" onClick={() => navigate('/profile')}>
            <X className="h-5 w-5" />
            <span className="sr-only">РћС‚РјРµРЅР°</span>
          </Button>
        </div>

        <p className="mt-4 max-w-2xl text-sm leading-7 text-muted-foreground sm:text-base">
          РћР±РЅРѕРІРёС‚Рµ СЂР°Р·РґРµР»С‹, РєРѕС‚РѕСЂС‹Рµ РІР»РёСЏСЋС‚ РЅР° РІР°С€Рё СЂРµРєРѕРјРµРЅРґР°С†РёРё Рё Р±СѓРґСѓС‰РёРµ СѓРІРµРґРѕРјР»РµРЅРёСЏ.
        </p>
      </FlatSection>
      ...
      <FlatSection className="mt-2">
        <p className="text-xs font-bold uppercase tracking-[0.24em] text-primary">Р’С‹Р±СЂР°РЅРѕ</p>
        <p className="mt-2 text-sm text-muted-foreground">
          {selectedTopics.length > 0 ? selectedTopics.map((topic) => topic.name).join(', ') : 'Р’С‹Р±РµСЂРёС‚Рµ С…РѕС‚СЏ Р±С‹ РѕРґРёРЅ СЂР°Р·РґРµР», С‡С‚РѕР±С‹ СЃРѕС…СЂР°РЅРёС‚СЊ РЅР°СЃС‚СЂРѕР№РєРё.'}
        </p>
      </FlatSection>
