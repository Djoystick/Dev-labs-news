create index if not exists post_reads_user_post_idx
on public.post_reads (user_id, post_id);

create index if not exists post_reads_user_created_at_idx
on public.post_reads (user_id, created_at desc);

create index if not exists post_reactions_post_value_idx
on public.post_reactions (post_id, value);
